...
sap.ui.define([
	...
	RotateToolGizmo.prototype._prepareForCreatingRotationKey = function(animationPlayer, time, playbackIndex) {
		...
		if (playback) {
			this._prepareForCreatingKey(playback.getSequence());
		}
	};
	...
	RotateToolGizmo.prototype._getNodesProperties = function() {
		...
		this._nodes.forEach(function(nodeInfo) {
			...
			if (userData && userData.eulerInParentCoors) {
				...
				if (this._sequence) {
					var sequenceOffset = this._viewport._viewStateManager._getEndPropertyInPreviousSequence(node, AnimationTrackType.Rotate, this._sequence);
					if (sequenceOffset) {
		...
		for (var ni = 0, nc = this._nodes.length; ni < nc; ni++) {
			...
			if (!nodeInfo.ignore) {
				...
				if (userData && userData.euler && userData.eulerInParentCoors) {
					...
					if (this._sequence) {
						var sequenceOffset = this._viewport._viewStateManager._getEndPropertyInPreviousSequence(node,
