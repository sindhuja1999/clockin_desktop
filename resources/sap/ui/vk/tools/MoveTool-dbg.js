...
sap.ui.define([
	...
	var MoveTool = Tool.extend("sap.ui.vk.tools.MoveTool", /** @lends sap.ui.vk.tools.MoveTool.prototype */ {
		metadata: {
			...
			events: {
				 ...
				 *		{float[]} offsetToRest,  translate relative to rest position in parent coordinates
	 			 * 		{float[]} offsetToPrevious,  translate relative to end position of previous sequence or
				 * 									 rest position if no previous sequence in parent coordinates
	 ...
	 *
	 * @param {sap.ui.vk.AnimationSequence} sequence current sequence for key creation
	 * @private
	 */
	MoveTool.prototype.prepareForCreatingTranslateKey = function(sequence){
		if (this._gizmo) {
			this._gizmo._prepareForCreatingKey(sequence);
			var nodesProperties = this._gizmo._getNodesProperties();
			this.fireEvent("moving", { x: 0, y: 0, z: 0, nodesProperties: nodesProperties }, true);
		}
	};
