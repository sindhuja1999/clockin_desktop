...
sap.ui.define([
	...
	var ScaleTool = Tool.extend("sap.ui.vk.tools.ScaleTool", /** @lends sap.ui.vk.tools.ScaleTool.prototype */ {
		metadata: {
			...
			events: {
				 ...
				 *		{float[]} offsetToRest,  scale relative to rest position in parent coordinates
	 ...
	 * @private
	 */
	ScaleTool.prototype.prepareForCreatingScaleKey = function(sequence){
		if (this._gizmo) {
			this._gizmo._prepareForCreatingKey(sequence);
			var nodesProperties = this._gizmo._getNodesProperties();
			this.fireEvent("scaling", { x: 1, y: 1, z: 1, nodesProperties: nodesProperties }, true);
		}
	};
