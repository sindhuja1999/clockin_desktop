...
sap.ui.define([
	...
	Billboard.prototype._ortho2DUpdate = function(renderer, camera) {
		...
		var z = Math.min(camera.near * (1 + originalTransform.s.length()), (camera.near + camera.far) * 0.5);
		this.position.copy(originalTransform.p).multiplyScalar(2 / camera.projectionMatrix.elements[ 5 ]);
		this.position.z = -1;
		this.position.multiplyScalar(z).applyMatrix4(camera.matrixWorld);
		this.quaternion.copy(camera.quaternion).multiply(originalTransform.q);
		this.scale.copy(originalTransform.s).multiplyScalar(z * 0.5);
		this.scale.z = Math.min(z - camera.near, this.scale.z * 0.001);
	...
	Billboard.prototype._lockToViewportUpdate = function(renderer, camera) {
		var scale = THREE.Math.lerp(camera.near, camera.far, 1e-4); // as close as possible to the near clipping plane
