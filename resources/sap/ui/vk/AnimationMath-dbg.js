...
sap.ui.define([
	...
	AnimationMath.neutralQuatToNeutralEuler = function(q, order) {
		var axis0 = order & 3;
		var axis1 = (order >> 2) & 3;
		var axis2 = (order >> 4) & 3;

		var value = [ 0, 0, 0, order ];

		var sinrCosp = 2 * (q.w * q.x + q.y * q.z);
		var cosrCosp = 1 - 2 * (q.x * q.x + q.y * q.y);
		var heading = Math.atan2(sinrCosp, cosrCosp);
		...
		if (Math.abs(sinp) >= 1) {
			...
			attitude = Math.asin(sinp);
		}

		// yaw (z-axis rotation)
		var sinyCosp = 2 * (q.w * q.z + q.x * q.y);
		var cosyCosp = 1 - 2 * (q.y * q.y + q.z * q.z);
		var bank = Math.atan2(sinyCosp, cosyCosp);

		value[axis0] = heading;
		value[axis1] = attitude;
		value[axis2] = bank;
	...
	AnimationMath.interpolate = function(valueType, keyBefore, keyAfter, k, track) {
		...
		switch (valueType) {
			...
			case AnimationTrackValueType.Euler:
				...
				// var r3j = new THREE.Euler(r[0], r[1], r[2]);
				// q = new THREE.Quaternion().setFromEuler(r3j);
				q = AnimationMath.neutralEulerToGlMatrixQuat(r);
				result.value = AnimationMath.glMatrixQuatToNeutral(q);
				// result.value = [ q.x, q.y, q.z, q.w ];
				break;
...
*/
				...
				result[AnimationTrackValueType.AngleAxis] = [ x, y, z, r ];
				...
				result.value  = quat.toArray();

				break;
			case AnimationTrackValueType.Vector3:
				result.value = [ AnimationMath.interpolateScalarLinear(valueBefore[0], valueAfter[0], k),
								AnimationMath.interpolateScalarLinear(valueBefore[1], valueAfter[1], k),
								AnimationMath.interpolateScalarLinear(valueBefore[2], valueAfter[2], k) ];
			break;
