...
sap.ui.define([
	...
	ActionUtil.prototype._initiateCall = function (mActionParams, bIsDraftEnabled) {
		...
		if (mActionParam != undefined && mActionParam.additionalParameters && mActionParam.additionalParameters.length == 0) {
			...
			var oFuncHandles = oEntityContext.map(function(oContext) {
			...
			Promise.all(contexts).then(function (oActionContexts) {
				...
				// set action context value to oModel for single selection & set default value(undefined) in case of multiple selection
