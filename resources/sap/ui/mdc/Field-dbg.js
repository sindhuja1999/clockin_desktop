...
sap.ui.define([
	...
	function _triggerConditionUpdate() {
		...
		if (!this.DELEGATE) {
			...
			this.oDelegatePromise.then(function() {_triggerConditionUpdate.call(this);}.bind(this));
			return;
		}
	...
	function _compareValues(vValue1, vValue2, bUpdateCheck) {
		...
		if (!bEqual && this.DELEGATE.getBaseType(this._oPayload, sDataType) === BaseType.Unit && Array.isArray(vValue1) && Array.isArray(vValue2)) {
	...
	function _initializeType(vValue) {
		...
		if (!this._bTypeInitialized) {
			if (!this.DELEGATE) {
				// wait until delegate is loaded
				this.oDelegatePromise.then(function() {_initializeType.call(this, vValue);}.bind(this));
				return;
			}

			var oBinding = this.getBinding("value");
			var oDataType = oBinding && oBinding.getType(); // use type from binding, not internal (might be a different one)
			this._oTypeInitialization = this.DELEGATE.initializeTypeFromBinding(this._oPayload, oDataType, vValue);
	...
	function _updateValue(aConditions) {
		...
		if (!this.DELEGATE) {
			// wait until delegate is loaded
			this.oDelegatePromise.then(function() {_updateValue.call(this, aConditions);}.bind(this));
			return;
		}
