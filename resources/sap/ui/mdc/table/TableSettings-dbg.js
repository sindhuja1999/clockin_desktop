	...
	var TableSettings = {
		...
		_getAdaptationController: function(oControl) {
			...
			if (!oControl._oAdaptationController) {
				oControl._oAdaptationController = new AdaptationController({
					...
					stateRetriever: function(TableDelegate){
						return TableDelegate.getCurrentState(oControl);
					},
		...
		createSort: function(oControl, sProperty, bRemoveAllExisting) {
			...
			sap.ui.require([
				...
				Delegate.getCurrentState(oControl).sorters.forEach(function(oProp) {
				...
				oAdaptationController.createSortChanges(aItems);
		...
		_moveItem: function(oControl, iDraggedIndex, iNewIndex, sMoveOperation) {
			sap.ui.require([
				...
				var aVisibleFields = Delegate.getCurrentState(oControl).items || [];
