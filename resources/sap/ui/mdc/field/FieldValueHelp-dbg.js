...
sap.ui.define([
	...
	var FieldValueHelp = FieldHelpBase.extend("sap.ui.mdc.field.FieldValueHelp", /** @lends sap.ui.mdc.field.FieldValueHelp.prototype */
	{
		...
		_sDefaultDelegate: "sap/ui/mdc/field/FieldValueHelpDelegate" // TODO: better way to set default
	});
	...
	function _checkSearchSupported() {
		...
		var oPromise = this.DELEGATE.determineSearchSupported(this.getDelegate().payload, this);
	...
	FieldValueHelp.prototype._getTextOrKey = function(vValue, bKey, oBindingContext, oInParameters, oOutParameters) {
		...
		if (oWrapper) {
			...
			if (!this.isOpen() && oBindingContext && oMyBindingContext !== oBindingContext &&
					((!oInParameters && this.getInParameters().length > 0) || (bKey && !oOutParameters && this.getOutParameters().length > 0))) {
			...
			vResult = SyncPromise.resolve().then(function() {
				if (bKey) {
					return oWrapper.getTextForKey(vValue, _mapInParametersToHelp.call(this, oInParameters), _mapOutParametersToHelp.call(this, oOutParameters, true));
				} else {
					// use default in-parameters for check
					return oWrapper.getKeyForText(vValue, _mapInParametersToHelp.call(this));
				}
...
//									}
	...
	function _toggleDefineConditions(bActive) {
		...
		if (oDialog && this._oField) {
			...
			if (bActive && !FilterOperatorUtil.onlyEQ(aOperators)) {
	...
	function _removeFilterBarFilters() {
		...
		if (oFilterBar) {
			var oFilterConditions = this._oFilterConditionModel.getAllConditions();
			var oFilterBarConditions = oFilterBar.getConditions();
			var sFilterFields = this.getFilterFields();
			...
			if (bUpdateFilterBar) {
				oFilterBar.setConditions(oFilterBarConditions);
			}
		}
