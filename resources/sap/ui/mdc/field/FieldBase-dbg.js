...
//				},
	...
	var mControlTypes = {
			Default: {
				...
				useDefaultFieldHelp: {name: "defineConditions", oneOperator: false} // if no FieldHelp and multiple operators use default FieldHelp
			},
			Search: {
			...
			Boolean: {
				...
				createDisplay: _createTextControl,
				useDefaultFieldHelp: {name: "bool", oneOperator: true} // if no FieldHelp and multiple operators use default FieldHelp
			},
			Unit: {
				edit: "sap/m/Input",
	...
	FieldBase.prototype.applySettings = function() {
		...
		if (!this.DELEGATE && !this._bDelegateSet) {
			this._oPayload = this.getDelegate().payload || {};
			this.loadDelegateModule();
		}

	};

	// enhance with delegate functions
	DelegateMixin.call(FieldBase.prototype);

	FieldBase.prototype.onBeforeRendering = function() {

		if (!this.DELEGATE) {
			// wait until delegate is loaded
			this.oDelegatePromise.then(function() {_createInternalContent.call(this);}.bind(this));
			return;
		}
	...
	function _createDataType(sType) {
		...
		sType = this.DELEGATE.getDataTypeClass(this._oPayload, sType);
		var TypeClass = _getDataTypeClass.call(this, sType);

		if (TypeClass) {
			var oType = new TypeClass(this.getDataTypeFormatOptions(), this.getDataTypeConstraints());
			oType._bCreatedByField = true;
			return oType;
		}
	...
	function _getDataType() {
		...
		if (!this._oDataType) {
			...
			if (typeof sDataType === "string") {
				this._oDataType = _createDataType.call(this, sDataType);
			}
		}
	...
	function _getDataTypeName() {
		...
		if (this._oDataType && typeof this._oDataType === "object") {
			...
			return this.DELEGATE.getDataTypeClass(this._oPayload, this.getDataType());
		} else {
	...
	function _getDatePattern(sType, oFormatOptions, oConstraints) {
		...
		var sBaseType = this.DELEGATE.getBaseType(this._oPayload, sType, oFormatOptions, oConstraints);
	...
	function _adjustDataTypeForUnit() {
		...
		if (!oFormatOptions || !oFormatOptions.hasOwnProperty("showMeasure") || oFormatOptions.showMeasure) {
			...
			this.DELEGATE.initializeInternalUnitType(this._oPayload, this._oDataType, this._oTypeInitialization);
			_updateConditionType.call(this);
		}
	...
	FieldBase.prototype._observeChanges = function(oChanges) {
		...
		if (oChanges.name === "dataType") {
			...
			if (this._oDataType) {
				var fnCheck = function (sType) {
					var oTypeClass = _getDataTypeClass.call(this, this.DELEGATE.getDataTypeClass(this._oPayload, oChanges.current));
				...
				if (!this.DELEGATE) {
					// wait until delegate is loaded
					this.oDelegatePromise.then(function() {fnCheck.call(this, oChanges.current);}.bind(this));
					return;
				}
		...
		if (oChanges.name === "delegate" && !this._bDelegateSet) { // TODO: Error if set twice?
			this._bDelegateSet = true;
			this._oPayload = oChanges.current.payload || {};
			this.loadDelegateModule.call(this);
		}
...
//			}
	...
	function _createInternalContent() {
		...
		var sBaseType = this.DELEGATE.getBaseType(this._oPayload, sDataType, oDataTypeFormatOptions, oDataTypeConstraints);
		...
		if (sControlName !== sControlNameOld) {
			this._bHideOperator = FilterOperatorUtil.onlyEQ(aOperators) && iMaxConditions === 1; // in single value eq Field hide operator
			...
			if (_useDefaultFieldHelp.call(this, oControlType, aOperators, sEditMode)) {
	...
	function _createDefaultFieldHelp(sType) {
		...
		if (!oFieldHelp) {
			...
			if (FieldHelp) {
				var oDelegate = this.DELEGATE && this.DELEGATE[mDefaultHelps[sType].getDelegate]();
	...
	function _useDefaultFieldHelp(oControlType, aOperators, sEditMode) {
		...
		if (oControlType.useDefaultFieldHelp && !this.getFieldHelp() && sEditMode !== EditMode.Display) {
			if (aOperators.length === 1) {
				...
				var bIsSingleValue = !oOperator || oOperator.isSingleValue(); // if operator not exists unse no field help
				...
				if (!(oControlType.editOperator && oControlType.editOperator[aOperators[0]]) &&
						(oControlType.useDefaultFieldHelp.oneOperator || !bIsSingleValue)) {
...
//		_handleConditionsChange.call(this, this.getConditions());
	...
	FieldBase.prototype._checkValueInitial = function(vValue) {
		...
		if (vValue === "") {
			...
			var sDataType = _getDataTypeName.call(this);
			if (this.DELEGATE.getBaseType(this._oPayload, sDataType) === BaseType.Unit
	...
	FieldBase.prototype._getOperators = function() {
		...
		var sBaseType = this.DELEGATE.getBaseType(this._oPayload, sDataType, oFormatOptions, oConstraints); // TODO what if delegate not loaded
