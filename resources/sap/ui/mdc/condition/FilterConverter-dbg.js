...
function(
	 ...
	 * @param {sap.ui.mdc.FilterBar} oFilterBar
	 * @returns {object} aConditionTypes map containing the types of the condition.
	 *
	 * @public
	 */
	FilterConverter.createConditionTypesMapFromFilterBar = function (oConditions, oFilterBar) {
		var oResult = {};

		for (var sFieldPath in oConditions) {
			if (oFilterBar) {
				var oPropertyInfo = oFilterBar._getPropertyByName(sFieldPath);
				var oDataType = oPropertyInfo && oPropertyInfo.baseType;
	 ...
	 * @param {function} [fConvert2FilterCallback]
	 * @returns {sap.ui.model.Filter} Filter object for filtering a listbinding
	 *
	 * @public
	 */
	FilterConverter.createFilters = function (oConditions, oConditionTypes, fConvert2FilterCallback) {
	...
	FilterConverter.prettyPrintFilters = function (oFilter) {
		var sRes;
		if (!oFilter) {
			return "";
		}
