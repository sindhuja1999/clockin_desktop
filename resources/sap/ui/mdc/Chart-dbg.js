...
sap.ui.define([
	...
	function (
		...
		Chart.prototype.initModules = function(aModules) {
			this.oDelegateModule = aModules[0];
		...
		Chart.prototype.applySettings = function(mSettings, oScope) {
			...
			if (mSettings) {
				...
				sDelegatePath = mSettings.delegate ? mSettings.delegate.name : this.getDelegate().name;
			}
			...
			.then(function onModulesLoaded(aModules) {
				...
				return this.oDelegateModule.fetchProperties(this);
			}.bind(this))
		...
		Chart.prototype._createInnerChart = function (mSettings, mItems) {
			...
			function addAdditionalColoringMeasures() {
				...
				for (var i = 0; i < aColMeasures.length; i++) {
					...
					if (aInSettings.indexOf(sKey) == -1) {
						mColorItem = this.oDelegateModule.retrieveAggregationItem("items", mItems[sKey]);
			...
			for (var i = 0; i < mSettings.items.length; i++) {
				...
				if (mItems[oItem.getKey()]) {
					mVizItemSettings = this.oDelegateModule.retrieveAggregationItem("items", mItems[oItem.getKey()]).settings;
				} else {
		...
		Chart.prototype._rebind = function() {
			...
			//TODO: Instead of doing one after the other we can do both in a single call
			//TODO: Also, this whole functionality should be moved into the base delegate
			var oListBinding = this.getAggregation("_chart").getBinding("data");
			var oSorters = this._getSorters();
			var oFilters = this._getFilterInfo();
			...
			if (oFilters && oFilters.filters) {
				oListBinding.filter(oFilters.filters);
			}
		...
		Chart.prototype._getFilterInfo = function() {
			var oFilterInfo = {};
			var oFilter = Core.byId(this.getFilter());
			if (oFilter) {
				oFilterInfo.filters = oFilter.getFilters();
				oFilterInfo.searchText = oFilter.getSearch(); // Probably not needed for chart
			}
			return oFilterInfo;
		};
		...
		Chart.prototype._onFiltersChanged = function() {
			if (this.isInnerChartBound()) {
