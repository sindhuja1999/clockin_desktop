...
sap.ui.define([
	...
	var TableDelegate = {
		...
		_getVisibleProperties: function(oTable) {
			...
			if (oTable) {
				oTable.getColumns().forEach(function(oMDCColumn, iIndex) {
		...
		_getSortedProperties: function(oTable) {
			return oTable.getSortConditions() ? oTable.getSortConditions().sorters : [];
		},
		...
		getCurrentState: function(oTable) {
			return {
				items: this._getVisibleProperties(oTable),
