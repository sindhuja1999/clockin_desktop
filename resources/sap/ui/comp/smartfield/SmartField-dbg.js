	...
	var SmartField = Control.extend("sap.ui.comp.smartfield.SmartField", /** @lends sap.ui.comp.smartfield.SmartField.prototype */ {
		metadata: {
			...
			events: {
				 ...
				 *
				 * <b>Note:</b> This event works only with currency fields and it's not fired for any other type
				 * of field.
				 */
				changeModelValue: {
					parameters: {
						...
						/**
						 * Indicates whether the currency unit field has changed
						 */
						unitChanged: {
							type: "boolean"
						},
						...
						/**
						 * Last value of the <code>valueState</code> property of the currency unit field
						 */
						unitLastValueState: {
							type: "sap.ui.core.ValueState"
						}
		...
		renderer: {
			...
			render: function(oRm, oControl) {
				var i, aInnerControls, oInnerControl, oContent = oControl.getContent();
				oRm.openStart("div", oControl);
				oRm.class("sapUiCompSmartField");
				oRm.openEnd();
				if (oContent && oControl.getMode() === "display" && oControl.isFormContextType()){
					aInnerControls =  oControl.getInnerControls();
					for (i = 0; i < aInnerControls.length; i++) {
						oInnerControl = aInnerControls[i];
						if (oInnerControl.isA("sap.m.Text") && oInnerControl.getText() === "" ||
							oInnerControl.isA("sap.ui.comp.smartfield.DisplayComboBox") && oInnerControl.getSelectedKey() === "") {
								oRm.openStart("span");
