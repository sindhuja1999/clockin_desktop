...
sap.ui.define([
    ...
    function GeneralEntry (viewId, viewFullName, viewType, entryHelpID, title, onSaveFunc, onCancelFunc, getContentFunc, getValueFunc, entryIcon, defaultVisibility) {
        this.view = null;
    ...
    function _getUserPrefDefaultModel () {
        ...
        if (Config.last("/core/shell/model/enableNotifications") === true) {
            ...
            oNotificationSettingsAvalabilityPromise.done(function (oStatuses) {
                if (oStatuses.settingsAvailable) {
                    ...
                    var aEntity = Config.last("/core/userPreferences/entries"),
                        oEntry;
                    for (var i = 0; i < aEntity.length; i++) {
                        oEntry = aEntity[i];
                        if (oEntry.title === resources.i18n.getText("notificationSettingsEntry_title")) {
                            oEntry.visible = true;
                            break;
                        }
                    }
                    Config.emit("/core/userPreferences/entries", aEntity);
                }
            });
        }

        if (Config.last("/core/shell/model/userDefaultParameters")) {
            sap.ui.getCore().loadLibrary("sap.ui.comp", { async: true }).then(function () {
                ...
                var allEntries = Config.last("/core/userPreferences/entries");
                allEntries.push(defaultParametersEntry);
                Config.emit("/core/userPreferences/entries", allEntries);
            });
        }
        ...
        return {
            enableNewComponent: Config.last("/core/userPreferences/enableNewComponent"),
        ...
        var oEntry = {
            ...
            contentFunc: function () {
                ...
                oSharedComponentUtils.getEffectiveHomepageSetting("/core/home/homePageGroupDisplay", "/core/home/enableHomePageSettings").then(function (sDisplay) {
                    flpSettingsView = sap.ui.xmlview("UserSettingsFLPSettingsView", {
     ...
     */
    function setModel () {
        var userPreferencesEntryArray = Config.last("/core/userPreferences/entries");
        ...
        Config.emit("/core/userPreferences", oDefaultUserPrefModel);
    }
