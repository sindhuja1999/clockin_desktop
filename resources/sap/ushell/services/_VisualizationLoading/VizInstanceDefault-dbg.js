...
sap.ui.define([
    ...
    oVisualization.prototype.isCustomTile = function () {
        if (!this.getAdapter().isCustomTile) {
            var oCatalogTile = this.getCatalogTile();
            var sComponentName = oCatalogTile.tileResolutionResult.tileComponentLoadInfo.componentName;
            return this.getAdapter()._isCustomTileComponent(sComponentName);
        }
    ...
    oVisualization.prototype._setVizViewControlPromise = function (vizData) {
        ...
        if (vizData && vizData.isBookmark) {
            this.oControlPromise = new Promise(function (resolve, reject) {
                // Bookmark tiles are completely stored in CommonDataModel's personalization delta and they are not known to the
                // (ABAP) FES server. Therefore, the server is not involved when loading the UI.
                vizData.chip.load(function () {
                    resolve(vizData.chip.getImplementationAsSapui5());
                });
            });
        } else {
    ...
    oVisualization.prototype.load = function () {
        ...
        var oVizLoadPromise = VizInstance.prototype.load.apply(this, arguments);
        if (!this.isCustomTile()) {
            oVizLoadPromise.then(function () {
                EventHub.emit("VizInstanceLoaded", that.getVisualizationId());
            });
        }
        return oVizLoadPromise;
    };
