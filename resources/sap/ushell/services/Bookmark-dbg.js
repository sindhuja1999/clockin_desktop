...
sap.ui.define([
    ...
    function Bookmark () {
        ...
        /**
         * Adds a bookmark tile to one of the user's home page groups in the launchpad homepage mode.
         *
         ...
         *   A <code>jQuery.Deferred</code> promise which resolves on success in launchpad homepage mode, but rejects
        ...
        this.addBookmark = function (oParameters, oGroup) {
            ...
            if (Config.last("/core/spaces/enabled")) {
                var sError = "Bookmark Service: The API 'addBookmark' is not supported in launchpad spaces mode.";
            ...
            oDeferred = oLaunchPageService.addBookmark(oParameters, oGroup);
            oDeferred.done(function (oTile) {
                var oData = {
                    tile: oTile,
                    group: oGroup
                };
        ...
        this.addBookmarkToPage = function (oParameters, sPageId) {
            ...
            if (!Config.last("/core/spaces/enabled")) {
                return oDeferred.reject("Bookmark Service: 'addBookmarkToPage' is not valid in launchpad homepage mode, use 'addBookmark' instead.").promise();
            }

            // Reject if personalization is disabled
            if (!Config.last("/core/shell/enablePersonalization")) {
                return oDeferred.reject("Bookmark Service: Add bookmark is not allowed as the personalization functionality is not enabled.").promise();
            }

            if (oParameters && (typeof oParameters.title !== "string" || typeof oParameters.url !== "string")) {
                return oDeferred.reject("Bookmark Service - Invalid bookmark data.").promise();
            }

            // Delegate to pages service
            sap.ushell.Container.getServiceAsync("Pages")
                .then(function (pagesService) {
                    pagesService.addBookmarkToPage(sPageId, oParameters);
                })
                .then(function () {
                    oDeferred.resolve();
                })
