...
sap.ui.define([
    ...
    GridContainer.prototype.addUrlForRightClick = function (oTileControl, sTarget) {
        var sOrigin = window.location.origin,
            sPathName = window.location.pathname,
            sUrlForLeanModeRightClick = "";

        if (sTarget) {
            // the new definition is that the href of a tile will open in "lean" state (in right click scenario)
            if (sTarget.charAt(0) === "#") {
                if (window.location.search && window.location.search.length > 0) {
                    sUrlForLeanModeRightClick = sOrigin + sPathName + window.location.search + "&appState=lean" + sTarget;
                } else {
                    sUrlForLeanModeRightClick = sOrigin + sPathName + "?appState=lean" + sTarget;
                }
            } else {
                sUrlForLeanModeRightClick = sTarget;
            }

            oTileControl.setUrl(sUrlForLeanModeRightClick);
        }
    };
    ...
    GridContainer.prototype.onBeforeRendering = function () {
        ...
        for (i = 0; i < aTiles.length; i++) {
            oTile = aTiles[i];
            sTarget = oTile.getTarget && oTile.getTarget();

            oTileViews = oTile.getAggregation("tileViews");
            oTileControl = oTileViews && oTileViews[0] && oTileViews[0].getTileControl && oTileViews[0].getTileControl();
            if (oTileControl) {
                this.addUrlForRightClick(oTileControl, sTarget);
            }
        }

        for (i = 0; i < aLinks.length; i++) {
            oLink = aLinks[i];
            oData = oLink.oModels.view && oLink.oModels.view.getData();
            sTarget = oData && oData.nav && oData.nav.navigation_target_url;
            this.addUrlForRightClick(oLink, sTarget);
        }
    };
