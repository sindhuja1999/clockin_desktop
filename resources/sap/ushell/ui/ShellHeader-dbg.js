...
sap.ui.define([
        ...
        function sendFocusToShellHeader () {
            if (that._accessKeyHandler.bForwardNavigation) {
                var aHeaderItems = that.getHeadItems(),
                    oLogo = that.getDomRef("logo");

                if (aHeaderItems.length > 0) {
                    aHeaderItems[0].focus();
                } else if (!that.isHomepage() && oLogo) {
                    oLogo.focus();
                } else {
    ...
    ShellHeader.prototype.isHomepage = function () {
        var sHash = window.hasher && "#" + window.hasher.getHash(),
            sHomeHash = this.getHomeUri();

        if (sHash && (sHomeHash === "" || sHomeHash === "#" || sHomeHash === "#Shell-home")) {
            var index = sHash.search(/\?/);

            if (index >= 0) {
                sHash = sHash.substring(0, index);
            }

            return sHash === sHomeHash || sHash === "#Shell-home" || sHash === "#Launchpad-openFLPPage";
        }

        return sHash === sHomeHash;
    };
