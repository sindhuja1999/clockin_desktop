...
sap.ui.define([
    ...
    return BaseDialogController.extend("sap.ushell.applications.PageComposer.controller.ContextSelector", {
         ...
         * stores an array of all available visualizations for the selected role context.
         *
         * @private
         */
        _writeSelectionToModel: function () {
            var aSelectedRoleIds = oList.getSelectedContexts(true).map(function (context) {
        ...
        openSelector: function (aRoles, aSelectedRoles, fnConfirm) {
            this.initRoles(aRoles, aSelectedRoles);
            this.attachConfirm(fnConfirm);
            return this.load().then(this.open.bind(this));
        }
    });
});
