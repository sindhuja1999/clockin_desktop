...
sap.ui.define([
	...
	return {
		...
		render: function (oRM, oNetworkGraph) {
			...
			// groups
			oRM.write("<div id=\"" + oNetworkGraph.getId() + "-divgroups\" class=\"sapSuiteUiCommonsNetworkGraphDivGroups\" aria-hidden=\"true\">");
			oRM.write("</div>");

			// nodes
			var sStyle = "style=\"opacity: 0\"";
			if (oNetworkGraph._isUseNodeHtml()) {
				oRM.write("<div id=\"" + oNetworkGraph.getId() + "-divnodes\" class=\"sapSuiteUiCommonsNetworkGraphDivNodes\" " + sStyle + " aria-hidden=\"true\">");
				oRM.write("</div>");
			}
