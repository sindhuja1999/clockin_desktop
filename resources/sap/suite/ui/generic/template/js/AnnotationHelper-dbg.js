sap.ui.define(["sap/ui/model/odata/AnnotationHelper",
	...
	"sap/ui/Global",
	"sap/ui/Device"
], function (AnnotationHelperModel, SmartField, Log, formatMessage, deepExtend, encodeXML, RuntimeFormatters, testableHelper, Device) {
	"use strict";
	...
	function fnGetFallbackId(sErrMsg) {
		...
		Log.error("Unable to generate Stable ID", sErrMsg, "FioriElements");
		var arr = new Uint32Array( 1 ), limit = Math.pow( 2, 32 );
		var nRandomNumber = window.crypto.getRandomValues( arr )[ 0 ] / limit;
...
*/
		...
		formatWithExpandSimplePath: function (oInterface, oDataField, oEntitySet) {
		...
		// Determine multi selection mode for tables
		...
		// Determine Delete mode for table
		getInlineDeleteForTable: function (oFacet, oSections) {
			var oSettings = oSections && oSections[oAnnotationHelper.getStableIdPartFromFacet(oFacet)];
			return oSettings && oSettings.tableSettings && oSettings.tableSettings.inlineDelete;
		},
		...
		getSelectionModeResponsiveTable: function (aEntities, sRootEntitySet, oManifestExt, oFacet, oSections, oEntitySet, oRouteConfig, bIsDraftEnabled, bGlobalMultiSelect) {
			//Check for selection mode of the table
			var bMultiSelect = oAnnotationHelper.getMultiSelectForTable(oFacet,	oSections, bGlobalMultiSelect),
				bInlineDelete = oAnnotationHelper.getInlineDeleteForTable(oFacet, oSections),
				sTableMode;

			if (bMultiSelect && bInlineDelete) {
				throw new Error("Both inlineDelete and multiSelect option for table are not possible");
			}

			if (bMultiSelect) {
				sTableMode = "MultiSelect";
			} else if (bInlineDelete) {
				sTableMode = "Delete";
			} else {
				sTableMode = "SingleSelectLeft";
			}

			if (!bInlineDelete && (oAnnotationHelper.hasActions(aEntities) || oAnnotationHelper.hasCustomActions(oRouteConfig, sRootEntitySet, oManifestExt, oFacet))) {
				return sTableMode;
			}
			var oDeleteRestrictions = oEntitySet["Org.OData.Capabilities.V1.DeleteRestrictions"];
			if (!oDeleteRestrictions || oDeleteRestrictions.Deletable && ((oDeleteRestrictions.Deletable.Bool && oDeleteRestrictions.Deletable.Bool !== 'false') ||	oDeleteRestrictions.Deletable.Path)) {
				return "{= " + ((bIsDraftEnabled) ? "" : "!") +	"${ui>/editable} ? '" + sTableMode + "' : 'None' }";
			}
			return "None";
		},
		...
		getStableIdPartFromFacet: function (oFacet) {
			...
			if (oFacet.RecordType && oFacet.RecordType === "com.sap.vocabularies.UI.v1.CollectionFacet") {
				if (oFacet.ID && oFacet.ID.String) {
					...
					sErrMsg = "Annotation Helper: Unable to create a stable ID. You have to set an ID at all collection facets.";
					return fnGetFallbackId(sErrMsg);
				}
				...
				sErrMsg = "Annotation Helper: Unable to create a stable ID. Please check the facet annotations.";
				return fnGetFallbackId(sErrMsg);
			}
		},
		...
		areBooleanRestrictionsValidAndPossible: function (oInterface, mRestrictions, oRelatedEntitySet, sRestrictionType, sSourceEntitySet, bOnlyValidityCheck) {
			...
			if (!bValid) {
				Log.error("Service Broken: Restrictions annotations for entity type " + sRelatedEntityType + " for section '" + sRestrictionType + "' are invalid.");
			}
		...
		setRowHighlight: function (oTreeNode, sListEntitySet, fnCheckIsDraftEnabled) {
			...
			// Note: The two last parameters will not be evaluated by the formatter. They are just contained to ensure that the formatter is called on every change of the message model and every change of the row context.
			// The row context is in particular important for scrolling in grid tables (recommendation from ui5).
			return "{parts: [" + sDraftParts + ", " + sMessageParts + ", {path: ''}], formatter: 'sap.suite.ui.generic.template.js.AnnotationHelper.setInfoHighlight'}";
		},
