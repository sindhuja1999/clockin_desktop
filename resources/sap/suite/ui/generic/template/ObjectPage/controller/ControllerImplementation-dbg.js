sap.ui.define([
	...
	var oMethods = {
		getMethods: function(oViewProxy, oTemplateUtils, oController) {
			...
			var oWaitForRebindPromise = null; // wait for this Promise to rebind in order to let UI5 update its internal structures first
			var bLayoutFinished; // this property will be set to true as soon as all layouts have been adapted (applyState) and reset to false when a new binding is applied (afterRebind)
			// end state
			...
			function adjustAndProvideBindingParamsForSmartTableOrChart(oEvent){
				...
				oBindingParams.parameters.transitionMessagesOnly = true;
			...
			function onActivateImpl() {
				...
				var oCRUDActionHandler = oBase.state.CRUDActionHandler;
				var oActivationPromise = oTemplateUtils.oServices.oCRUDManager.activateDraftEntity(oCRUDActionHandler);
			...
			function performAfterSaveOperations(oContext, oView, bSomethingWasChanged, mPendingChangesToContextInfo, bStayInEdit) {
				...
				if (bCreateMode) {
					if (bStayOnPage) { // stay on the item -> UI5 has adapted the binding context of the view
						// however, the url (and internal information) needs to be adapted
			...
			function onSave(sSaveCase, sBtnId){
				...
				oTemplateUtils.oCommonUtils.executeIfControlReady(oBase.state.CRUDActionHandler.handleCRUDScenario.bind(null, iScenario, fnImpl), sButtonId);
			}
			...
			function getObjectPageDeleteDialog() {
				fnTableDeleteConfirmationOnDelete = function(oEvent) {
					...
					var oDeleteEntityPromise = oTemplateUtils.oServices.oCRUDManager.deleteEntity(null, null, oCRUDActionHandler);
			...
			*/
			...
			function fnDeleteEntriesImpl(oButton) {
				...
				oBeforeLineItemDeleteExtensionPromise.then(function(oExtensionResult) {
					extend(oDialogParameter, oExtensionResult);
					// get Delete Confirmation Popup fragment
					getTableDeleteDialog(oButton, oSmartTable).then(function (oDialog) {
			...
			function fnLazyLoad(bMustWaitForHeader, bIsGettingVisible, oSubSection, oSubSectionInfoObject){
				...
				if (bIsGettingVisible){
					...
					// if bMustWaitForHeader is true we actually even wait for a call where bLayoutFinished has already been set to true
					if (oSubSectionInfoObject.fnLazyLoadResolve && (bLayoutFinished || !bMustWaitForHeader)){
						oSubSectionInfoObject.fnLazyLoadResolve();
						delete oSubSectionInfoObject.fnLazyLoadResolve;
					}
					return;
				}
				var aWaitForPromises = [oWaitForRebindPromise];
				...
				Promise.all(aWaitForPromises).then(function(){
					...
					oSubSectionInfoObject.refresh(null, true, true);
			...
			var mStrategiesForVisibilityChangeOfSubsection = {
				...
				reuseComponent: function(bIsGettingVisible, oSubSection) {
					if (bLayoutFinished || !bIsGettingVisible){
			...
			function fnRecalculateSectionStates(oSelectedSectionInfo) {
			...
			function fnHandleUITableExpand(oSmartTable, bSetFitContainer) {
				...
				while (oParent) {
					if (oParent.isA("sap.ui.layout.Grid")) {
						...
						Log.warning("Unexpected parent " + oParent.toString() + " in fnHandleUiTableExpand method", "ControllerImplementation");
					}
			...
			function fnOnSwitchTabs(bForward) {
				...
				for (var i = 0; i < aSections.length; i++) {
					...
					if (sSwitchingItemKey === sCurrentKey) {
						...
						var oSelectedSectionInfo = {
							section : oController.byId(sSectionId),
							sectionId : sSectionId
						};
						fnSectionChanged(oSelectedSectionInfo);
						break;
					}
				}
			}

			function fnSectionChanged(oSelectedSectionInfo) {
				fnRecalculateSectionStates(oSelectedSectionInfo);
				oBase.stateChanged();
			}
			...
			function fnLayoutFinished(){
				...
				Log.info("Call of _triggerVisibleSubSectionsEvents", "", "sap.suite.ui.generic.template.ObjectPage.controller.ControllerImplementation (fnLayoutFinished)");
				oObjectPage._triggerVisibleSubSectionsEvents(); // ensure that all lazyLoad-strategies waiting for layouting to be finished will be executed
			}
			...
			oViewProxy.beforeRebind = function(oWaitForPromise) {
				oWaitForRebindPromise = oWaitForPromise;
				if (!fnStateAppliedAfterBindResolve){
			...
			oViewProxy.afterRebind = function() {
				Log.info("Call of _triggerVisibleSubSectionsEvents", "", "sap.suite.ui.generic.template.ObjectPage.controller.ControllerImplementation (afterRebind)");
			...
			oViewProxy.applyHeaderContextToSmartTablesDynamicColumnHide = function(oContext){
				...
				oTemplateUtils.oCommonUtils.executeForAllInformationObjects("smartTableWithDynamicColumnHide", fnExecuteDynamicColumnHide.bind(null, true));
			};
			...
			var oControllerImplementation = {
				onInit: function() {
					...
					var oSelectedSectionInfo = fnGetSelectedSectionInfo();
					fnRecalculateSectionStates(oSelectedSectionInfo); // Initialize the Active SubSections
