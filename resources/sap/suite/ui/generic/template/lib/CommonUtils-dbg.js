sap.ui.define(["sap/ui/base/Object",
	...
	function getMethods(oController, oServices, oComponentUtils) {
		...
		// sap.suite.ui.generic.template.lib.CommonUtils.prototype.getSelectedContexts.getDialogFragment below
		function getDialogFragmentAsync(sName, oFragmentController, sModel, fnOnFragmentCreated) {
		    return oServices.oApplication.getDialogFragmentForViewAsync(oController.getView(), sName, oFragmentController, sModel, fnOnFragmentCreated);
		}
		...
		*/
				...
				if (bHasExternalChanges || oModel.hasPendingChanges()){
					...
					return new Promise(function(fnResolve, fnReject) {
						fnDataLossConfirmation(function(){
							...
							fnResolve(fnProcessFunction());
						},	function(){
							fnReject(fnCancelFunction());
						}, sMode, false);
		...
		function fnProcessDataLossConfirmationIfNonDraft(fnProcessFunction, fnCancelFunction, oState, sMode, bNoBusyCheck){
			var oComponent = oController.getOwnerComponent();
			var oAppComponent = oComponent.getAppComponent();
			var bSuppressPopup = oAppComponent.suppressDataLossPopup();
			if (bSuppressPopup){
				return bNoBusyCheck ? fnProcessFunction() : oServices.oApplication.performAfterSideEffectExecution(fnProcessFunction);
			} else if (bNoBusyCheck){
				return fnProcessDataLossConfirmationIfNonDraftImpl(fnProcessFunction, fnCancelFunction, oState, sMode, true);
			}
			return oServices.oApplication.performAfterSideEffectExecution(fnProcessDataLossConfirmationIfNonDraftImpl.bind(null, fnProcessFunction, fnCancelFunction, oState, sMode, false));
		}
		...
		*/
		function fnDataLossConfirmation(onDataLossConfirmed, onDataLossCancel, sMode, bIsTechnical) {
			...
			var oDataLossPopup;
			getDialogFragmentAsync(sFragmentname, {
			...
			}, "dataLoss").then(function name(oPopup) {
