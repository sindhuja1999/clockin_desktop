...
], function(BaseObject, routingHelper, testableHelper, Log, extend, deepExtend, isEmptyObject) {
	"use strict";
	...
	*/
	function getMethods(oTemplateContract, oSettings) {
		...
		function fnStoreCurrentAppStateAndAdjustURL() {
			if (!bAppStateDirty || sRealizedAppStateKey !== sAppStateKeyInUrl){ // if nothing to do or last update has not yet finished return
				return;
			}
