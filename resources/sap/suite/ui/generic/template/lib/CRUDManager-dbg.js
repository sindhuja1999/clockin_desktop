sap.ui.define(["sap/ui/base/Object", "sap/m/MessageToast", "sap/ui/generic/app/util/ModelUtil",
	...
	function(BaseObject, MessageToast, ModelUtil, ActionUtil, MessageUtils, MessageBox, CRUDHelper, testableHelper, extend) {
		...
		function getMethods(oController, oComponentUtils, oServices, oCommonUtils, oBusyHelper) {
			...
			//oCRUDActionHandler is a optional parameter which will determine how warnings should be handled.
			//oCRUDActionHandler may show a dialog, so the returned Promise must not be used for setting the app busy.
			function getDeleteEntityPromise(bIsActiveEntity, bHasActiveEntity, oContext, bDeleteDraftForActiveEntity, bSuppressSuccessMessage, oCRUDActionHandler) {
				var oRet = new Promise(
					function(resolve, reject) {
						...
						if (oCRUDActionHandler) {
							var fnHandleWarning = function(errorCode) {
								bWarningOccured = true;
								oBusyHelper.getUnbusy().then(function() {
									var fnForceDelete = function() {
										var oDeletePromise = oServices.oTransactionController.deleteEntity(oContext,null,true);
										oDeletePromise.then(function(oResponse) {
											fnHandleSuccess();
											resolve(oResponse);
										} ,function(oError) {
											MessageUtils.handleError(MessageUtils.operations.deleteEntity, oController, oServices, oError, null);
											reject();
										});
										oBusyHelper.setBusy(oDeletePromise);
									};
									oCRUDActionHandler.handleCRUDScenario(4, fnForceDelete, reject, "Delete");
								});
							};
						}
						if (bIsActiveEntity && bDeleteDraftForActiveEntity) {
							...
							var oDeletePromise = oServices.oTransactionController.deleteEntity(oContext, null, !oCRUDActionHandler);
							oBusyHelper.setBusy(oDeletePromise);
							oDeletePromise.then(
								function(oResponse) {
									fnHandleSuccess();
			 ...
			 * @public
			 */
			function deleteEntity(bDeleteDraftForActiveEntity, bSuppressSuccessMessage, oCRUDActionHandler){
				var oRet = new Promise(function(fnResolve, fnReject){
					...
					oSiblingPromise.then(function(oActive){
						var oDeletePromise = getDeleteEntityPromise(bIsActiveEntity, bHasActiveEntity, oContext, bDeleteDraftForActiveEntity, bSuppressSuccessMessage, oCRUDActionHandler);
			 ...
			 * @public
			 */
			function deleteEntities(aPath, bSuppressRefreshAllComponents) {
				...
				var oRet = new Promise(function(resolve, reject) {
					...
					oServices.oApplication.prepareDeletion(mObjectsToDelete, bSuppressRefreshAllComponents);
		...
		//		handleError.bind(null, MessageUtils.operations.saveEntity, reject));
			 ...
			 * @public
			 */
			function activateDraftEntity(oCRUDActionHandler, oCreateDialogContext) {
				if (oBusyHelper.isBusy()) {
					return oRejectedPromise;
				}
				var oRet = new Promise(function(resolve, reject) {
			...
			function addEntry(oTable, oPredefinedValues) {
				...
				oCreatePromise.catch(handleError.bind(null, MessageUtils.operations.addEntry, function(oError){throw oError;}));
