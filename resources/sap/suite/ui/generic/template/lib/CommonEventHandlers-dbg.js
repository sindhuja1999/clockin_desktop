	...
	function getMethods(oController, oComponentUtils, oServices, oCommonUtils) {
		...
		function getDeleteDialog(smartTable) {
			...
			return oCommonUtils.getDialogFragmentAsync("sap.suite.ui.generic.template.ListReport.view.fragments.DeleteConfirmation", {
				...
				onDelete: function(oEvent) {
					...
					// prepare delete promise
					var oDeletePromise;
					if (isFclWithNextObjectLoad() || isMultipleView()) {
						// Referesh all the active component after the delete promise
						oDeletePromise = oServices.oCRUDManager.deleteEntities(aPathsToBeDeletedDraftActive, false);
					} else {
						oDeletePromise = oServices.oCRUDManager.deleteEntities(aPathsToBeDeletedDraftActive, true);
						// Referesh the table with same changeset name to merge the get call with above delete action
						oCommonUtils.refreshSmartTable(smartTable, 'Changes');
					}
			...
			return new Promise(function(fnResolve, fnReject) {
				Promise.all(aPromise).then(function(aResponses) {
					...
					if (mJSONData.itemsCount > 1) {
						...
						} else if (sTableMode === "Delete" && mJSONData.undeletableCount > 0) {
		...
		*/
		...
		function fnSemanticObjectLinkPopoverLinkPressed (oEvent, oState){
			// TODO: remove when UI5 fix navigation issue. BCP: 2070088953
...
//			}
		...
		function deleteEntry(oEvent) {
			...
			var aSelectedItems = [oListItemContext];
			deleteImpl(oSmartTable, aSelectedItems);
		}
