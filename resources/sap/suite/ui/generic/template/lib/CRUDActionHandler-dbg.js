 ...
 */
 ...
 sap.ui.define(["sap/ui/base/Object", "sap/ui/model/Filter", "sap/ui/model/FilterOperator", "sap/base/util/extend"
	...
	function getMethods(oTemplateContract, oController, oCommonUtils) {
		...
		function getConfiguredPopoverIfNeeded(iSituation, sCRUDAction){
			return new Promise(function (fnResolve) {
				...
				oCommonUtils.getDialogFragmentAsync("sap.suite.ui.generic.template.fragments.MessageInfluencingCRUDAction", {
					...
					if (iSituation === 3){
						...
						oLocalModel.setProperty("/title", oRet.getModel("i18n").getProperty("ST_DETAIL_TITLE_SAVE"));
						if (oItemBinding.filter(oErrorFilter).getLength() === 0) {
							oLocalModel.setProperty("/situation", 4); // 4: no Error, only Warning
		...
		function fnRestartCRUDAction(fnOperationCallback, fnCancelCallback, sCRUDAction){
			fnYes = fnOperationCallback;
			fnNo = fnCancelCallback;
			getConfiguredPopoverIfNeeded(3, sCRUDAction).then(function (oWarningDialog) {
				if (oWarningDialog) {
					oWarningDialog.open();
				} else {
					fnCancelCallback();
		...
		function fnHandleCRUDScenario(iScenario, fnPerformOperation, fnCancel, sCRUDAction) {
			if (iScenario === 4) {
				fnRestartCRUDAction(fnPerformOperation, fnCancel, sCRUDAction);
			} else {
