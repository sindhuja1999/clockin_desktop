	...
	function(BaseObject, FlexibleColumnLayoutSemanticHelper, fioriLibrary, extend) {
		...
		function getMethods(oFlexibleColumnLayout, oNavigationControllerProxy) {
			...
			function isLayoutDefault(sLayout, sRoute){
				...
				var sDefaultLayoutTypeIfExternalNavigation = oTreeNode && oTreeNode.defaultLayoutTypeIfExternalNavigation;
				// only in case of external navigation we want to check if the layout currently present in sLayout matches the default layout for external navigation.
				// In truthy I would classify sLayout as default as it's value is equivalent to Default Layout type in External Navgation and this is case of external nav.
				if (oNavigationControllerProxy.isExternalNav() && sDefaultLayoutTypeIfExternalNavigation){
					return sLayout === sDefaultLayoutTypeIfExternalNavigation;
				}
			...
			// returns the layout which need to be set in case of external Navigation.
			function getFCLLayoutInExternalNavigation(oIdentity) {
				return oNavigationControllerProxy.isExternalNav() && oIdentity.treeNode.defaultLayoutTypeIfExternalNavigation;
			}

			// returns the layout which is used when this identity is active
			function getFCLLayoutFromIdentity(oIdentity){
				return getFCLLayoutFromStateObject(oIdentity.appStates) || getFCLLayoutInExternalNavigation(oIdentity) || oIdentity.treeNode.defaultLayoutType || aDefaultLayouts[tVL(oIdentity.treeNode.fCLLevel)];
			}
