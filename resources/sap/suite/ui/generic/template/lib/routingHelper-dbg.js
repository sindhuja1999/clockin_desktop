...
sap.ui.define(["sap/ui/model/Filter",
	...
	"sap/base/util/each"
		...
		function fnEnhanceEntityNode(mEntityTree, sEntitySet, oDraftContext) {
			...
			if (oEntityNode.draftSpec === "parent") {
				if (oEntityNode.parent) {
					fnEnhanceEntityNode(mEntityTree, oEntityNode.parent, oDraftContext);
		...
		function fnEnhanceEntityTree(oNavigationControllerProxy, sRootEntitySet) {
			...
			oMetaModel.loaded().then(function() {
				...
				fnEnhanceEntityNode(mEntityTree, sRootEntitySet, oDraftContext);
				for (var sEntitySet in mEntityTree) {
					fnEnhanceEntityNode(mEntityTree, sEntitySet, oDraftContext);
				}
			});
		}
		...
		function fnHandleEmbeddedComponents(oTreeNode, oRoute, iLevel, oNavigationControllerProxy, sTargetControlId, iParentFCLDistanceToRoot) {
			...
			if (oRoute.implementingComponent) {
				fnHandleEmbeddedComponent(oTreeNode, iLevel, oNavigationControllerProxy, sTargetControlId, iParentFCLDistanceToRoot, "implementation", oRoute.implementingComponent, "template::ImplementingComponent");
		...
		function createRoutes(vPredecessorTargets, oPage, iLevel, oParentNode, oNavigationControllerProxy, sTargetControlId, aNodes,
			...
			if (oPage.component) { //in case of intent there is no internal route to be created
				...
				var oTreeNode = {
					...
					defaultLayoutTypeIfExternalNavigation: oPage.defaultLayoutTypeIfExternalNavigation,
				...
				var oNewRoute = createRoute(vPredecessorTargets, oPage, oPage.component.list ? "aggregation" : "detail", oTreeNode, oParentNode,
		...
		function addPathFunctionToTreeNode(oTreeNode, oParentNode, bCreateRequestsCanonical){
			...
			if (oTreeNode.noOData){
				...
				sBindingPath = bCreateRequestsCanonical ? sCanonicalPath : "/" + sRoutePattern;
			}
		 ...
		 */
		function createRoute(vPredecessorTargets, oPage, sOperation, oTreeNode, oParentNode, oNavigationControllerProxy, sTargetControlId) {
			...
			if (iLevel > 1) {
				oNewRoute.name = oParentNode.sRouteName + "/" + (oParentNode.embeddedComponent ? oParentNode.embeddedComponent + "/" : "") + oNewRoute.name;
			}
					...
					case "create":
						...
						if (bIsDraft){
							...
							oCreatePromise.then(function(oContext) {
								oNavigationControllerProxy.navigateToContext(oContext, "", true, 4).then(fnInitRouter);
							}, function(oError) {
								fnInitRouter();
							...
							oNavigationControllerProxy.navigateForNonDraftCreate(sEntitySet, oPredefinedValues).then(fnInitRouter);
						}
						break;
